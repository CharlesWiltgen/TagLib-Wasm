...
| **MP4 / M4A**                                                                          | `audio-payload` | the contents of every non-empty top-level `mdat`, in file order. Layout atoms are outside: a `free` atom before the first `mdat`, a `moov` at EOF, and the `moof` headers of a fragmented file — whose `mdat`s are included in file order like any other                                   | an atom size is malformed or oversized, bytes are left that no atom accounts for, or there is no non-empty `mdat`                                             |
...
| **An input TagLib cannot read** (a zero-byte buffer, junk bytes, an unknown extension) | —               | nothing is reported: the failure differs by input kind and by backend                                                                                                                                                                                                                      | —                                                                                                                                                             |
...
An input TagLib cannot read fails by input kind, and the difference matters
before a checksum is treated as evidence. A **path** is opened and the walk runs,
so the honest fallback answers for it: `empty.mp3` gives `source: "file"` with
`bytesHashed: 0`, and a junk `.mp3` gives the file's whole length — a corrupt
file gets a whole-file checksum rather than an error. A **buffer**
(`Uint8Array`, `ArrayBuffer`, `File`) is refused before the walk: zero bytes
throw `WasmMemoryError`, a non-empty junk buffer throws `InvalidFormatError`. A
path with an extension TagLib does not recognize throws `WasmMemoryError`, and
under `setBufferMode(true)` all of these throw `InvalidFormatError`. None of them
reports a `source`.
...
<!-- Walk trace (rows above): src/taglib/media-ranges.ts — `walkMpeg` (ID3v2 skip,
     ...
     by tests/media-ranges.test.ts. The last row is `TagLib.open`'s error
     surface, measured per input kind — not a walk. -->
