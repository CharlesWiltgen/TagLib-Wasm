...
/**
 * MP4 genre-precedence fixtures (taglib-lna2), generated by
 * `tests/test-files/_gen/make-gnre-first-mp4.py` and consumed by
 * `tests/mp4-genre-precedence.test.ts`. Each derives from `mp4/kiss-snippet.m4a`
 * with one ilst edit: a numeric `gnre` atom (ID3v1 index 18 -> "Rock") sits on
 * either side of a differing string ©gen ("Rock & Roll"), is dropped (`omit`,
 * the control proving ©gen parses on its own), or is left alone (`only`, the
 * table name as the sole genre).
 */
export const GNRE_GENRE_FIXTURES = {
  before: resolve(TEST_FILES_DIR, "mp4/genre-gnre-before.m4a"),
  after: resolve(TEST_FILES_DIR, "mp4/genre-gnre-after.m4a"),
  omit: resolve(TEST_FILES_DIR, "mp4/genre-gnre-omit.m4a"),
  only: resolve(TEST_FILES_DIR, "mp4/genre-gnre-only.m4a"),
} as const;
