...
describe("encodeTagData key transformation", () => {
  it("should encode acoustidFingerprint as ACOUSTID_FINGERPRINT", () => {
    const tag: ExtendedTag = { acoustidFingerprint: "AQADtNQYhYkYRcg" };
    const encoded = decode(encodeTagData(tag)) as Record<string, unknown>;
  ...
  it("should encode discNumber as DISCNUMBER", () => {
    const tag: ExtendedTag = { discNumber: 2 };
    const encoded = decode(encodeTagData(tag)) as Record<string, unknown>;
  ...
  it("should pass through unknown keys as-is", () => {
    const tag = { title: "Test", CUSTOM_FIELD: "value" } as ExtendedTag;
    const encoded = decode(encodeTagData(tag)) as Record<string, unknown>;
